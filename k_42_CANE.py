...
# Varijanta: CANE – context-aware (embedding čvora + kontekst suseda)

"""
Graphs in Space: Graph Embeddings for Machine Learning on Complex Data.
CANE (Context-aware Network Embedding): bazično ugrađivanje E_self (PPMI/SVD),
zatim kontekst = ponderisani prosek suseda; E(i) = (1−α)·E_self(i) + α·E_context(i).
"""
...
CANE_ALPHA = 0.4 # 0.4 alpha for CANE
CANE_T = 5       # 5 steps for CANE
...
def compute_cane_embeddings(M, k=EMBED_DIM):
    ...
    if tot < 1e-12:
        tot = 1.0
    P_mat = S / tot
    P_row = P_mat.sum(axis=1, keepdims=True)
    P_col = P_mat.sum(axis=0, keepdims=True)
    ppmi = np.log2((P_mat + 1e-12) / (P_row * P_col + 1e-12) + 1e-12)
    ppmi = np.maximum(ppmi, 0.0)
    U, S_svd, _ = np.linalg.svd(ppmi, full_matrices=False)
    n_comp = min(k, len(S_svd))
    E_self = U[:, :n_comp] * np.sqrt(np.maximum(S_svd[:n_comp], 1e-12))
    if E_self.shape[1] < k:
        E_self = np.hstack([E_self, np.tile(E_self[:, -1:], (1, k - E_self.shape[1]))])
    E_context = np.zeros_like(E_self)
    for i in range(39):
        w = P[i, :]
        w = w / (w.sum() + 1e-12)
        E_context[i] = w @ E_self
    emb = (1.0 - CANE_ALPHA) * E_self + CANE_ALPHA * E_context
    for d in range(k):
        col = emb[:, d]
...
def main():
    ...
    emb = compute_cane_embeddings(M, k=EMBED_DIM)
    ...
    print()
    print("Predikcija (CANE + kvantna regresija, deterministički, strukturno):")
    print(best_combo)
    print()
    print("Score:", pred_scores[best_combo[0]])
    print()
    """
    Predikcija (CANE + kvantna regresija, deterministički, strukturno):
    (5, 8, 11, 22, 23, 28, 33)

    Score: 0.4915081923795594
    """
...
E_self: 
PPMI od S = I + P + … + P^CANE_T (CANE_T=5), 
SVD → prvih k kolona U√Σ (39×k).

Kontekst: 
za svaki čvor i, 
E_context(i) = ponderisani prosek E_self(j) po susedima j, 
težine = red P(i,:) (normalizovane) (39×k).

Spajanje: 
E(i) = (1 − CANE_ALPHA)·E_self(i) + CANE_ALPHA·E_context(i) (CANE_ALPHA=0.4) (39×k).
